...
将 **CMS-2026** 加解密算法带入KernelSU、APatch、Magisk等管理器中，支持CLI命令操作和WebUI界面

---

## 功能介绍

* **✅ CLI支持:** 通过终端（Termux 或 ADB Shell）输入 `cms2026` 即可调用
* **✅ 系统级注入:** 将加密工具集成至 `/data/local/bin/` 和 `/data/local/cms2026/`
...
# 从文件读取输入
cms2026 -e -i input.txt -p "密码"

# 输出到文件
cms2026 -e "内容" -p "密码" -o output.enc
...
| 选项 | 说明 |
|:-----|:-----|
| `-e, --encrypt <text>` | 加密给定文本 |
| `-d, --decrypt <text>` | 解密给定密文 |
| `-p, --password <pwd>` | 设置加密/解密密码 |
| `--strong` | 使用强加密模式 (256位密钥) |
...
```bash
# 示例 1: 基本加密
$ cms2026 -e "Hello World" -p "secret123"
CMS2026:2.0:B:xxxxxx:xxxxxx:xxxxxxxxx

# 示例 2: 解密
$ cms2026 -d "CMS2026:2.0:B:xxxxxx:xxxxxx:xxxxxxxxx" -p "secret123"
Hello World

# 示例 3: 强加密
$ cms2026 -e "重要数据" -p "strongpass" --strong
CMS2026:2.0:S:xxxxxx:xxxxxx:xxxxxxxxx

# 示例 4: 文件加解密
$ cms2026 -e -i secret.txt -p "pass" -o secret.enc
$ cms2026 -d -i secret.enc -p "pass" -o decrypted.txt
```
...
* 如果CLI命令无法使用，请检查 `/data/local/bin/` 是否在PATH中，或直接使用完整路径 `/data/local/bin/cms2026`
